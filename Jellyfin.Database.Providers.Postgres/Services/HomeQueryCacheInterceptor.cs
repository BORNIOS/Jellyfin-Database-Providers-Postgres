...
public sealed class HomeQueryCacheInterceptor : DbCommandInterceptor
{
	...
	/// <summary>The singleton set after construction, for cache purge operations.</summary>
	...
	/// <summary>Initializes a new instance of <see cref="HomeQueryCacheInterceptor"/>.</summary>
	public HomeQueryCacheInterceptor(ILogger? logger = null)
	{
		...
		{
			// Cap at 200 entries: at ~15 cached queries per TTL window × sensitivity margin,
			// this never exceeds ~10 MB even with large result sets.
			SizeLimit = 200,
		});
	}
		...
		CancellationToken cancellationToken = default)
	{
		...
		{
			_logger?.LogDebug("HomeQueryCache HIT: {SqlPreview}",
		...
		CancellationToken ct)
	{
		try
		{
			...
			_logger?.LogDebug("HomeQueryCache MISS (cached {Cols} cols x {Rows} rows, key {KeyPreview}…): {Sql}",
	...
	private static string? RewriteRandomSql(string sql)
	{
		...
		var tableMatch = Regex.Match(sql,
		...
		var rewritten = Regex.Replace(sql,
